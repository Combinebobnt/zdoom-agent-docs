...
**Tier:** A (both claims traced directly to the relevant `PCD_*` opcode implementations in `p_acs.cpp`, not wiki-sourced or inferred).
**Engine:** Zandronum 3.2.1 (verified against the Zandronum source `master` HEAD, a `3.3-alpha` development snapshot ahead of the 3.2.1 target — this is core VM instruction behavior, stable across that gap).
...
engine; it sets the script's ACS-VM `state` to `SCRIPT_ModulusBy0` (i.e. it silently terminates/
suspends that script's execution at that instruction rather than throwing a catchable error).
