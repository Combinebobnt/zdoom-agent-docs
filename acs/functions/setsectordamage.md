...
**Provenance:** `SetSectorDamage - ZDoom Wiki` (`_intake/SetSectorDamage - ZDoom Wiki.html`, retrieved 2026-07-29, `https://zdoom.org/w/index.php?title=SetSectorDamage&oldid=44305`). Wiki's signature/parameter defaults are recorded above for reference, but the wiki's described *behavior* does not hold in Zandronum at all — verified absent against Zandronum source (`p_acs.cpp:5360-5500` `EACSFunctions` enum — no `94`/`ACSF_SetSectorDamage` member exists between explicit `ACSF_Warp=92` and `ACSF_GetActorFloorTexture=204`; `p_acs.cpp:9059-9063` default-case fallthrough; `p_acs.cpp:9461-9473` `PCD_CALLFUNC` doing no funcIndex validation before dispatch) and against `zt-bcc` codegen (`zt-bcc/src/parse/dec.c:2453-2493` confirms the leading `-` in `-94:SetSectorDamage(...)` is consumed as a type-selector flag, not baked into the stored `id` — so the emitted `funcIndex` really is the positive literal `94`, matching the engine-side enum numbering exactly). Empirically confirmed compiling: a script calling `SetSectorDamage(1, 5, "Fire", 32, 0);` built to a `.o` via `bcc` with exit code 0 and no diagnostics. Cross-checked working alternatives `Sector_SetDamage` (`p_lnspec.cpp:2459-2473`) and `SectorDamage` (`functions/sectordamage.md`).
**Wiki license:** Derived from the ZDoom Wiki; this file as a whole is GNU Free Documentation License 1.2 — see [LICENSE](../../LICENSE) §2.
**Bucket:** extension function (by declared table position; real implementation on UZDoom's side, no engine side on Zandronum's — see above).

**Implemented on UZDoom, matching the wiki's described behavior exactly.** `case
ACSF_SetSectorDamage:` (`src/playsim/p_acs.cpp`) loops every sector matching the `tag` argument
(`Level->GetSectorTagIterator`) and sets `sec->damageamount`, `sec->damagetype` (as an `FName`,
default `NAME_None`), `sec->damageinterval` (clamped `1..INT_MAX`, default `32`), and
`sec->leakydamage` (default `0`) directly — a genuine persistent per-sector property setter, with
defaults matching this doc's wiki-sourced Parameters section below exactly (`interval` default 32,
`leaky` default 0). Declared in `zt-bcc/lib/zcommon.bcs:1724` as
`-94:SetSectorDamage(int,int;str,int,int):void` — a negatively-indexed extension function
(`ACSF_SetSectorDamage`, funcIndex `94`).

**Does not exist in Zandronum's engine.** The same declaration compiles cleanly against Zandronum's
`zcommon.bcs` table too (confirmed empirically: a test script calling
`SetSectorDamage(1, 5, "Fire", 32, 0);` compiled to a `.o` with `bcc`, exit code 0), but there is
no `ACSF_SetSectorDamage` enumerator, and no `case` for funcIndex `94`, anywhere in the Zandronum
source's `src/p_acs.cpp`'s `EACSFunctions` enum or its `CallFunction` dispatch `switch` (starts
`p_acs.cpp:5899`).

## What actually happens at runtime on Zandronum
...
`ACSF_GetActorFloorTexture = 204`; nothing in Zandronum's enum has value `93`-`203`, including
`94`. The `switch`'s only fallback is a bare `default: break;` (`p_acs.cpp:9059-9060`), after which
the function returns `0` (`p_acs.cpp:9063`). So calling
...
## Zandronum-only workarounds (UZDoom just has the real function — see above)
...
- **`Sector_SetDamage(tag, amount, mod)`** — action special, positive index `214`
  ...
  knobs from the wiki page have no equivalent on Zandronum.
...
Neither Zandronum alternative reproduces `SetSectorDamage`'s exact contract (persistent + string
damage type + configurable interval + leaky-suit probability); `Sector_SetDamage` is persistent but
loses the string type and both timing knobs, `SectorDamage` keeps the string type but is one-shot,
not persistent. **On UZDoom, none of this is needed — `SetSectorDamage` itself is the real,
persistent, full-contract implementation.**

## Parameters

Per the wiki, confirmed matching UZDoom's real implementation (`sec->damageamount`/`damagetype`/
`damageinterval`/`leakydamage` assignment in `case ACSF_SetSectorDamage:` — not exercised at all
on Zandronum, see above):

- `tag` — sector tag, per the wiki.
- `amount` — damage amount, per the wiki.
- `damagetype` — default `"None"` per the wiki; confirmed on UZDoom as an `FName`-lookup damage
  type, matching `SectorDamage`'s `type` param. No Zandronum equivalent.
- `interval` — tics between damage applications, default `32` per the wiki, confirmed on UZDoom
  (clamped `1..INT_MAX`). No Zandronum equivalent (`Sector_SetDamage` has no interval concept, and
  the sector damage tic-cadence is otherwise handled by the base `P_PlayerInSpecialSector` path,
  not by anything this function would set).
- `leaky` — radiation-suit leak probability (`0`-`256`), default `0` per the wiki, confirmed on
  UZDoom (stored directly as `sec->leakydamage`, no range-clamping observed in the assignment
  itself). No Zandronum equivalent either.

## Not to be confused with

`SectorDamage` (a real, working, differently-shaped compiler builtin) — see its doc's
