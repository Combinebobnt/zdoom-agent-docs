...
- `delay` — **tics before the platform returns to its original height.** Passed through the
  ...
  tics on this fork due to truncating integer division, so 105 tics is ~2.94 seconds, not exactly
  3.
- `lip` — **mandatory here, unlike the wiki's phrasing.** The compiled signature in `zcommon.bcs:1545`
  ...
  here (missing the mandatory `lip` arg). **All actual values in this fork need all four.**
