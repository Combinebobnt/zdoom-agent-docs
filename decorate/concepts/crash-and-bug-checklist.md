...
**Engine:** Zandronum 3.2.1 (inherits each linked finding's own `Engine:` stamp; check the
individual file if a future engine retarget is in question).
...
fork's DECORATE layer — not a tutorial, not exhaustive. Each entry is one sentence of "what to
...
1. **A NULL-actor-pointer dereference hidden behind short-circuit `&&`/`||` evaluation order,
   ...
   *before* the trailing `inv->IsKindOf(AAmmo)` check even runs, and with no NULL guard on
   ...
   of what item is being taken. See [A_TakeInventory](../actions/a_takeinventory.md).
...
1. **6 of the 11 ZDoom-wiki `CHF_*` flags for `A_Chase` aren't defined anywhere in Zandronum's own
   ...
   code has no case for that bit, so it's silently a no-op. A ZDoom-wiki page describing a DECORATE
   feature is not proof the feature exists in this fork — see [A_Chase](../actions/a_chase.md) for
   the exact bit values and the general engine-family-gate reminder in
   `../../shared/AUTHORING.md`'s "Engine scope" for why this class of gap keeps recurring across
   every ZDoom-wiki intake, not just this one flag set.

2. **A bitflag-parameter action function's wiki-documented constant list includes names this fork
   ...
   — none of these are declared in `wadsrc/static/actors/constants.txt`, so passing one compiles
   and runs as an inert integer with no matching `case`/`if` in `InitSpawnedItem`. See
   ...
   checked against `wadsrc/static/actors/constants.txt` (or the matching native `enum` in the
   implementing `.cpp`) directly — never assume a wiki's flag table is this fork's flag table, even
...
1. **`A_SpawnItemEx`'s `SXF_TRANSFERPOINTERS`-assigned `target` can be silently overwritten.** For
   ...
   actor with a live `player->attacker`, `InitSpawnedItem` unconditionally sets
   `mo->target = attacker` *after* `SXF_TRANSFERPOINTERS` already ran, with no flag to suppress it.
   A monster spawned this way only keeps a `SXF_TRANSFERPOINTERS`-assigned `target` if the
...
2. **`A_SpawnItemEx`'s `SXF_SETMASTER` sets `master` to the *originator*, not literally the calling
   actor, and does nothing at all when the originator is a player.** A minion spawned from a
   projectile gets the projectile's shooter as its master (not the projectile itself); if the
   originator resolves to a player rather than a monster, `SXF_SETMASTER` has no effect on `master`
   whatsoever — that code path only touches friendliness and `target`. See the same file's
   `SXF_SETMASTER` entry.
...
[Jump functions and network synchronization](network-jump-synchronization.md) — the three items
below are the reviewable, grep-for-this-in-the-diff form of that file's causes 1–3, kept here so a
...
1. **RNG inside an `A_JumpIf` (or similar non-`A_Jump`) condition, on an actor that isn't
   ...
   this fork) — the real cost is quietly less-reproducible `+CLIENTSIDEONLY` cosmetic randomness on
...
3. **A position/LOS/inventory-gated jump (`A_JumpIfCloser`, `A_JumpIfTargetInLOS`,
   ...
   value by up to one round-trip under this fork's predict-and-correct model, so an exact-tic
   ...
   changes.
4. **Same wasted-roll shape as #1 above, outside the jump-function family: an attack action function
...
2. **`A_JumpIfMasterCloser`/`A_JumpIfTracerCloser` have no client-mode guard, unlike their sibling
   ...
   which this fork does not reliably replicate. A client can reach a different jump decision than
