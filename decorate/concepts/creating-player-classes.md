...
**Provenance:** ZDoom Wiki "Creating new player classes" (retrieved 2026-07-31, oldid=52200), verified against the Zandronum source's DECORATE actor definition examples (`wadsrc/static/actors/doom/doomplayer.txt`), `Player.*` property definitions (`src/thingdef/thingdef_properties.cpp:2254-2836`), MAPINFO parser (`src/gi.cpp:330-331`), KEYCONF command definitions (`src/p_user.cpp:233-241`), and multiplayer player-class selection mechanism (`src/d_netinfo.cpp`, `src/menu/multiplayermenu.cpp`, `src/menu/menudef.cpp:1164-1257`).

Player classes allow a game to define multiple player character variants (different sprites, stats, weapon sets, appearance) selectable by the player, either at new-game startup (single-player) or when joining a game (multiplayer). This page covers the DECORATE-side mechanisms — actor definition and property configuration — and describes how Zandronum's multiplayer player-class selection differs from the single-player workflow described in the wiki.

## Defining a player class in DECORATE

A player class is defined by creating a new DECORATE actor inheriting from `PlayerPawn` or a subclass like `DoomPlayer` (the shipped Doom player character, defined in `wadsrc/static/actors/doom/doomplayer.txt`):
...
- **`Player.Colorset <index>, "<name>", <r1>, <r2>, <color>"` (and further per-palette variants)** — Predefined color translations by name, allowing the player-setup menu to offer discrete color choices by name (e.g., "Red", "Blue", "Green") instead of a continuous palette range. Not required if using `Player.ColorRange` alone.
...
- **`Player.WeaponSlot <slot>, <weapon1>, <weapon2>, ...`** — Assigns weapons to inventory slots (1–10) for quick access. Slot 0 is reserved. Example: `Player.WeaponSlot 1, Fist, Chainsaw` puts the Fist and Chainsaw in slot 1.
- **`Player.ViewHeight <units>`** — The camera height when standing. Defaults to 41 units; humanoid players typically use 41–56.
- **`Player.MaxHealth <hitpoints>`** — The maximum health the player can reach with powerups (not the `Health` property, which is the initial health). If not set, defaults to `Health`.
- **`Player.JumpZ <units>`** — Jump force; higher values allow higher jumps (only used if the player has the `+JUMP` actor flag or if jumping is explicitly enabled in a custom script).
...
- **`Player.StartSpawnClass "<className>"`** — If set, causes this player to spawn a spawner of the specified actor type instead of the player itself at level start. Rarely used.
...
The wiki page describing "Choosing player classes in-game" focuses on single-player ZDoom. Zandronum's multiplayer-focused architecture handles class selection differently:
...
- Player class is stored as a **`playerclass` userinfo cvar** (a network-synchronized player property like `name`, `skin`, or `color`), not selected during a new-game flow (`src/d_netinfo.cpp:90`).
- The player class takes effect **at spawn/respawn time**: when the player joins the server or respawns, the server looks up the player's `CurrentPlayerClass` from userinfo and spawns the corresponding `PlayerPawn` actor (`src/p_user.cpp:677-681`).
- The class **remains in effect for the duration of the game or until explicitly changed** via the player-setup menu, console command, or explicit userinfo update. **Mid-game class switching is possible** if the player changes their `playerclass` userinfo during play; the change takes effect at the next respawn (behavior determined by server-side spawn logic in `src/p_user.cpp`).
- A **multiplayer "join game" menu** offers a class selector when the player is about to join, allowing class selection as part of the join process (visible in the "Player Class chooser for the join menu" in `src/menu/menudef.cpp:916`).
...
- **How exactly is a class changed mid-game in multiplayer?** The code path from menu selection or console `set playerclass` to the next respawn with the new class is not fully traced. Specifically: does the new class take effect immediately if the player is already alive, or only on the next respawn? Is there a server-side throttle or validation of class changes during active play?
- **Does the shipped class-selection menu support `+NOMENU` classes?** The menu builder (`src/menu/menudef.cpp`) checks the `PCF_NOMENU` flag and excludes such classes (line 1225, `if (!(PlayerClasses[i].Flags & PCF_NOMENU))`), but whether this flag is separately settable in DECORATE (beyond automatically inheriting from the actor's `MF6_NOMENU` flag) is not verified.
- **Zandronum-fork-specific class-handling cvars?** This checkout has `sv_forcerandomclass` and `gameinfo.norandomplayerclass` visible (e.g. `src/team.cpp:1635`, `src/menu/menudef.cpp:1238`). There may be other server-side cvars affecting class selection or availability that aren't documented here.
