...
This tree ships two ready-made Agent-tool subagent definitions in `agents/`:

- **`zdoom-docs-lookup`** (retrieval, read-only, `model: haiku`) — answers ACS/BCS/DECORATE/
  ZScript/lump-format questions by walking this tree itself, following the same routing this file
  describes. If it's registered, delegate a lookup question to it directly instead of working
  through the "Where to go" table and section `INDEX.md`s by hand.
- **`zdoom-docs-intake`** (processing, `model: haiku`) — turns one saved wiki page under
...
copied into the calling project's own `.claude/agents/`. If neither is registered, `agents/*.md`
here is the canonical source: register it yourself (copy or symlink into a `.claude/agents/`
directory, project- or user-level) or paste its contents into a generic subagent's prompt as a
one-off. Every section's own `CLAUDE.md` repeats a one-line pointer back to this section, since an
...
**Zandronum is the primary target** (current: Zandronum 3.2.1). ZScript and some DECORATE/MAPINFO/
GLDEFS/SBARINFO/CVARINFO behavior only exists on UZDoom/GZDoom-family engines — every doc's
`Engine:` field says which. See `shared/AUTHORING.md`'s "Engine scope" for the full detail,
including known gaps between the local checkouts and their own upstreams. Don't assume a doc
verified for one engine holds for another.
